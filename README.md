...
| [`frontend-sdk-contract.yaml`](.github/workflows/frontend-sdk-contract.yaml) | Consumer pin for browser apps that generate an HTTP SDK. Checks out `inboxxhq-api-contracts` and runs [`scripts/check-frontend-sdk-contract.py`](scripts/check-frontend-sdk-contract.py) so `api-contract.lock.json` + the committed OpenAPI snapshot are the published `<service>/<version>.json`, byte for byte. Then regenerates the Angular SDK from that pin (`generate-from-lock.js --check`) so adapters cannot keep importing a stale client. Runtime `/api/docs-json` is not a contract source (ADR-0067). Thin-called from each app's `ci.yaml`. |
...
| [`runner-toolbox-contract.yaml`](.github/workflows/runner-toolbox-contract.yaml) | Runner toolbox contract — runs [`scripts/runner-toolbox-contract.sh`](scripts/runner-toolbox-contract.sh), the executable list of every tool the reusable workflows assume of a runner (the `ubuntu-latest` tool set, python3 + yaml, `gh`, `shellcheck`, `envsubst`, a C toolchain that compiles and links, cgo enabled as Go itself reports it, the scale-set layout), on a **live** `inboxxhq-ci` runner. `workflow_call` only: this repository is public and a runner group does not admit public repositories, so the check is run by thin callers in private repositories, one per organization (`inboxxhq-infra` runs it daily and on demand for the InboxxHQ-CoderAxis pool; the CoderAxis pool needs the same caller in a private CoderAxis repository). The image build (`inboxxhq-infra` `ci-workflows-runner/runner-image/build.sh`) runs the identical script inside the image before pushing, so a tool the fleet needs cannot go missing from the image, and cannot drift out of the deployed pool, without a red run naming it. Adding a tool to the image means adding its probe to the script in the same change. |
...
| DOC-0025 | A Markdown document outside the `normative_homes` list in `controls/log-schema.yaml` MUST NOT state a normative rule about log records. It cites the owning document instead. A statement is normative when it prescribes a field name, a field type, a timestamp format or timezone, a formatter, an output stream, a log level policy, a PII or redaction rule, or a Loki label or structured-metadata assignment - typically detectable by an imperative (`must`, `MUST`, `required`, `never`) within a sentence naming one of those subjects. Per-service `docs/operations/LOGGING_REFERENCE.md` files are in scope and are permitted to record only that service's configured values and its declared deviations. | major | document | always | platform-sre | active |
...
| Control | Policy | Severity | Scope | Owner | Status |
| ------- | ------ | -------- | ----- | ----- | ------ |
| WFC-0001 | A consumer repository MUST NOT contain a workflow whose filename matches a workflow that coderaxis/ci-workflows publishes as reusable (`on.workflow_call`) unless that workflow is a caller of it — that is, unless it contains `uses: coderaxis/ci-workflows/.github/workflows/<name>@<ref>`. | major | caller-workflow | platform-infrastructure | active |
| WFC-0002 | A call to a coderaxis/ci-workflows reusable workflow MUST reference a major version tag (`@v1`, `@v2`, ...). It MUST NOT reference a branch (`@main`), an exact patch tag (`@v1.6.0`), or a commit SHA. | major | caller-workflow | platform-infrastructure | active |
...
| SP-0006 | Every service's Kubernetes `spec.ports[].port`, `targetPort`, `containerPort` and probe ports MUST be 8080 (HTTP), 50051 (gRPC) and 9464 (metrics). | critical | platform-architecture | active |
...
| SP-0012 | A Deployment MUST name the HTTP bind port `PORT`. `HTTP_PORT` is a retired spelling (RFC-0007 §3, SYNC-019) and MUST NOT appear. | critical | platform-architecture | active |
| SP-0011 | A service whose code calls `servicediscovery` MUST require platform-shared-go v1.48.0 or later, the first release whose registry answers cluster resolution with the RFC-0007 §4.1 constants rather than a generated per-service port. | critical | platform-architecture | active |
...
| SP-0013 | `localports.yaml` MUST exist and give every service an `http`, `grpc` and `metrics` port; no entry may be a §4.1 cluster constant or a §3 reserved port; no two services may share a non-zero port within a concern; and `ports.generated.go`'s `localPorts` table MUST agree with it. | critical | platform-architecture | active |
| SP-0014 | Where a repository has an `env.example`, its `PORT`, `GRPC_PORT` and `METRICS_PORT` MUST equal that service's entry in `localports.yaml`. | major | platform-architecture | active |
